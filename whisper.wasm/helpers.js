...
var printTextarea = (function() {
    ...
    return function(text) {
        ...
        console.log(text);
        const match = text.match(/\[\d{2}:\d{2}:\d{2}\.\d{3} --> \d{2}:\d{2}:\d{2}\.\d{3}\]/);

        if (match) {
            const timestamp = match[0];
            const matchEndIndex = text.indexOf(timestamp) + timestamp.length;
            const textAfter = text.slice(matchEndIndex).trim();
            //console.WriteLine("TEXT: " + textAfter);
            document.getElementById('resultOutput').value += textAfter + "\n";
            document.getElementById('resultOutput').scrollTop = document.getElementById('resultOutput').scrollHeight;
        } else {
            if (element) {
                element.value += text + "\n";
